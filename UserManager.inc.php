...
{	
	...
	*/
	...
	function account_create(bool $confirm, string $user, string $pass, string $email = NULL, string $nom = NULL, string $prenom = NULL, string $adresse = NULL, string $ville = NULL, string $code_postal = NULL):bool{
		if($confirm != TRUE)
	...
	function account_connect(bool $confirm, string $user, string $pass):bool{
		...
		if ($userok == FALSE)
			return FALSE;
		if(hash_verif(TRUE, $pass, $pass_verif)) {
			@session_start();
	...
	function account_disconnect(bool $confirm):bool{
		if($confirm != TRUE) trigger_error("Il faut confirmer la fonction", E_USER_ERROR);
		session_start();
		session_regenerate_id();
		return TRUE;
	...
	function account_mod(bool $confirm, string $user = "", string $nom = "", string $prenom = "", string $email = "", string $cp = "", string $ville = "", string $adresse = ""):bool{
		if($confirm != TRUE) trigger_error("Il faut confirmer la fonction", E_USER_ERROR);
		$verif = account_verif();
	...
	function account_modmdp(bool $confirm, string $mdp):bool{
		if($confirm != TRUE) trigger_error("Il faut confirmer la fonction", E_USER_ERROR);
		if($mdp === "") trigger_error("Mot de passe n'est pas renseignée.", E_USER_ERROR);
		$verif = account_verif();
		if(@$verif[connect] == FALSE) return FALSE;
		$mdpcrypt = hash_create($mdp);
		include(__DIR__."/UserManagerConfig.inc.php");
		$mysqli_connect = mysqli_connect();
	...
	function account_clearsession():bool{
		include(__DIR__."/UserManagerConfig.inc.php");
		$mysqli_connect = mysqli_connect();
	...
	function accountrecup_create(bool $confirm, string $email, string $user):string{
		if($confirm != TRUE) trigger_error("Il faut confirmer la fonction", E_USER_ERROR);
	...
	function accountrecup_use(bool $confirm, string $token):string{
		if($confirm != TRUE) trigger_error("Il faut confirmer la fonction", E_USER_ERROR);
	...
	function return_list(int $nb):string{
		if ($nb === 1) return "Mauvais nom d'utilisateur ou mot de passe.";
		return NULL;
	}

	function admin_createtoken(bool $confirm):bool{
		if($confirm != TRUE) trigger_error("Il faut confirmer la fonction", E_USER_ERROR);
		include ("UserManagerConfig.inc.php");
		$token = hash_create(uniqid());
		$mysqli_connect = mysqli_connect();
	...
	function admin_usetoken(bool $confirm):bool{
		if($confirm != TRUE) trigger_error("Il faut confirmer la fonction", E_USER_ERROR);
		include ("UserManagerConfig.inc.php");
		$mysqli_connect = mysqli_connect();
	...
	function admin_connectuser(bool $confirm, string $user):bool{ // ATTENTION: Fonction sensible : Permet une connexion d'un utilisateur sans mot de passe.
		if($confirm != TRUE) trigger_error("Il faut confirmer la fonction", E_USER_ERROR);
		if($user == "") trigger_error("User n'est pas renseignée.", E_USER_ERROR);
	...
	function modassist_create(bool $confirm, string $user, bool $mod_info = FALSE, bool $mod_mdp = FALSE):int{
		if($confirm != TRUE) trigger_error("Il faut confirmer la fonction", E_USER_ERROR);
		if($user == "") trigger_error("User n'est pas renseignée.", E_USER_ERROR);
	...
	function modassist_use(bool $confirm, string $token, int $pin):bool{
		
	}

	*/

	function hash_create(string $pass):string{
		if($pass == "") trigger_error("Mot de passe n'est pas renseignée.", E_USER_ERROR);
		include 'UserManagerConfig.inc.php';
	...
	function hash_verif(bool $confirm, string $pass, string $pass_verif){
		if($confirm != TRUE) trigger_error("Il faut confirmer la fonction.", E_USER_ERROR);
		if($pass == "") trigger_error("Mot de passe n'est pas renseignée.", E_USER_ERROR);
